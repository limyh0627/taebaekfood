...
const OfficeTalk: React.FC<OfficeTalkProps> = ({
  ...
  useEffect(() => {
    if (activeRoomId) {
      ...
      fetchInitialMessages(activeRoomId);
    }
  }, [activeRoomId]);
  ...
  const loadMoreMessages = async () => {
    if (!activeRoomId || !lastDoc || isLoadingMore || !hasMore) return;
    ...
    try {
      ...
      setLocalMessages(prev => [...msgs, ...prev]);
      setLastDoc(snapshot.docs[snapshot.docs.length - 1] || null);
      setHasMore(snapshot.docs.length === PAGE_SIZE);
    } catch (error) {
  ...
  const handleSendMessage = (e?: React.FormEvent, imageUrl?: string) => {
    e?.preventDefault();
    if ((!messageText.trim() && !imageUrl) || !activeRoomId) return;
    ...
    socketRef.current?.emit('send_message', { roomId: activeRoomId, message: newMessage });
              ...
              {hasMore && (
                <div className="flex justify-center py-2">
                  <button 
                    ...
                    className="flex items-center space-x-2 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-indigo-600 transition-colors disabled:opacity-50"
                  >
                    {isLoadingMore ? <Loader2 size={14} className="animate-spin" /> : <ChevronUp size={14} />}
                    <span>이전 대화 불러오기</span>
                  </button>
                </div>
              )}

...
| 품목 | 변경 전 | 변경 후 | 근거 |
|---|---|---|---|
| `p-1784787834504` 참기름/병/특/알찬/350ml (낱개) | stock 0 | stock **900** | `ORD-1786405645013`(해피유통(쿠팡)) 900개가 2026-08-11 작업완료(DISPATCHED)·미출고인데 재고 +900이 반영 안 됨. 이 품목 참조 주문은 그 1건뿐이라 다른 차감 경로 없음. 사용자 확인: 900병 실물 보유 |
...
- ⚠️ **재발 가능(값 정정은 대증요법)**: 박스 음수 2건은 **박스를 낱개에서 즉석 포장해 출고하면서 출고(−)만 잡히고 생산(+)은 안 잡히는** 배선 문제. 코드 수정 없이는 다시 음수로 흐름. 별도 과제.
- ⚠️ 알찬350도 **작업완료 시 완제품 재고 +N이 왜 누락됐는지는 미규명** — 재발하면 `orderStockEngine.produceOrder` 경로 점검 필요.

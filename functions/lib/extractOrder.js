...
exports.extractOrder = (0, https_1.onCall)({ region: REGION, secrets: [ANTHROPIC_API_KEY], timeoutSeconds: 60 }, async (request) => {
    ...
    const { text, catalog, partners, history, today } = ((_a = request.data) !== null && _a !== void 0 ? _a : {});
    const 글 = String(text !== null && text !== void 0 ? text : '').trim().slice(0, 글자한도);
    if (!글)
        throw new https_1.HttpsError('invalid-argument', '읽을 글이 없습니다.');
    const 품목목록 = (Array.isArray(catalog) ? catalog : []).slice(0, 품목한도);
    if (!품목목록.length)
        throw new https_1.HttpsError('invalid-argument', '품목 목록이 비어 있습니다.');
    ...
    const 오늘 = /^\d{4}-\d{2}-\d{2}$/.test(String(today !== null && today !== void 0 ? today : '')) ? String(today) : new Date().toISOString().slice(0, 10);
    const 본문 = [
        지침(오늘),
        '',
        '## 품목 목록 (id\t이름)',
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', '주문 읽기 서버에 연결하지 못했습니다.');
    }
...
//# sourceMappingURL=extractOrder.js.map
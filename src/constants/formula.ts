...
export const PRODUCT_FORMULA: Record<string, { raw: string; ratio: number }[]> = {
  ...
  /*
   * 생들기름은 **볶지 않고 짜낸 것**이라 수입산 들기름과 원료가 아예 다르다.
   * 그런데 시골향생들기름 3종이 품목키를 '시골향들기름2'(수입산 100%)로 달고 있었다.
   * BOM에는 생들기름이 붙어 있는데 배합표는 수입들기름을 가리켜, 원가도 서류상 사용량도
   * 엉뚱한 원료에서 빠졌다(원가는 12,566/kg짜리를 11,467/kg로 봤다).
   */
  '시골향생들기름': [{ raw: '생들기름', ratio: 1.0 }],

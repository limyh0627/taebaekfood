...
describe('PRODUCT_FORMULA — 서류용 품목 배합 (2026-08-09 확정)', () => {
  ...
  it('들기름', () => {
    ...
    expect(mix('해달들기름')).toEqual({ 통들깨들기름: 0.1, 수입들기름: 0.9 });
    expect(mix('하남댁맑음들기름')).toEqual({ 통들깨들기름: 0.5, 수입들기름: 0.5 });
  });
...
describe('시골향생들기름 서류 배합', () => {
  const mix = (k: string) => Object.fromEntries(PRODUCT_FORMULA[k].map(r => [r.raw, r.ratio]));
  it('시골향생들기름과 시골향들기름2 모두 수입산 100%', () => {
    expect(mix('시골향들기름2')).toEqual({ 수입들기름: 1.0 });
    expect(mix('시골향생들기름')).toEqual({ 수입들기름: 1.0 });
  });
});

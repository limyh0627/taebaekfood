 ...
 */
/**
 * 서류에 적는 규격 — **개입수 1은 규격이 아니다.**
 *
 * 품목의 spec은 개입수를 달고 있다: 낱개 '1kg * 1', 박스 '1kg * 20'.
 * 그런데 생산판매일지 양식은 용량만 쓴다 — '1kg'. 그대로 맞대면 낱개가 죄다 어긋난다.
 *
 *   시골향볶음참깨 '1kg * 1' 41개  →  양식의 '1kg' 칸에 안 붙어 '기타(템플릿 외)'로 밀렸다.
 *   기름은 밀리지도 않았다 — 템플릿이 등록 품목에서 자동으로 늘어나는 바람에
 *   '1800ml'(늘 0)과 '1800ml * 1'(실제 수량)이 **두 줄로 갈려** 섰다.
 *
 * 박스('* 20')는 그대로 둔다. 개입수가 규격의 일부라 낱개와 갈라야 한다.
 */
export const docSpec = (spec?: string): string =>
  String(spec ?? '').replace(/\s*\*\s*1\s*$/, '').trim();

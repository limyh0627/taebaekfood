 ...
 *
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 *  ⚠ 「원료수불부」와 「원장」은 **다른 것이다.** 헷갈리지 말 것.
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 *
 *  ┌─ 원장 (rawMaterialLedger 컬렉션) ─────────────────────────────────────────┐
 *  │  실물 재고 기록. 로트와 한 몸으로 움직인다.                                  │
 ...
 *
 *  ┌─ 원료수불부 (서류) ───────────────────────────────────────────────────────┐
 *  │  관청 제출용 문서. 원장에서 가져오는 건 **입고뿐**이다.                        │
 *  │  · 사용량     : 판매에서 되계산 (docSaleLine → docOilKg → addOilByRaw)      │
 *  │  · 입고       : 원장 그대로 ← 원장에서 오는 유일한 값                        │
 *  │  · 실사       : **서류 전용 실사**(rawDocEntries) — 원장 실사는 안 본다       │
 *  │  · 전월이월   : **전달 서류의 기말재고** — 원장 누적이 아니다                  │
 *  │  · 날짜 기준  : 배송완료일(docDateOf) — 판매기록부와 같은 날                  │
 *  │  · 만드는 곳  : buildRawDocSheetFor() ↓                                    │
 *  └──────────────────────────────────────────────────────────────────────────┘
 *
 *  두 축이 같은 날짜·같은 양일 필요는 없다. 원장은 실제 차감 시점, 서류는 판매 시점이다.
 *  서류에 원장의 자동차감(rm-auto)·실사는 절대 넣지 않는다.
 *
...
/**
 * 서류 기준일 — **배송완료일(deliveredAt)** 하나만 본다.
 *
 * 네 서류(판매기록부·원료수불부·생산작업기록부 1·2)가 같은 날짜를 봐야 물량이 같은 날에 잡힌다.
 * 그 기준을 실제로 물건이 나간 날 하나로 못 박았다.
 *
...
// ── 원료수불부(서류) 한 장 만들기 ────────────────────────────────────────────
// ⚠ 원장(rawMaterialLedger)과 다른 것이다. 서류가 원장에서 가져오는 건 **입고뿐**이다.
//
//   사용량    판매에서 되계산 (oilOutflow) — 원장의 자동차감(rm-auto)은 안 본다
//   입고      원장 그대로
//   실사      **서류 전용 실사**(rawDocEntries) — 원장 실사는 안 본다
//   전월이월  **전달 서류의 기말재고** — 원장 누적이 아니다
//
// 전월이월이 재귀라 첫 달만 서류 전용 이월(type:'opening')을 넣어 주면 그 뒤는 저절로 이어진다.

/** 서류 전용 기록 — 실사와 첫 달 이월. 원장과 별개 컬렉션(rawDocEntries)에 산다. */

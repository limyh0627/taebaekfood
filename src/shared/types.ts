...
/**
 * **출고 방식 — 어떻게 나가는가.** 판매 채널(`OrderSource`)과 **다른 축**이다.
 * 이름은 사장님이 정하셨다(2026-09-12): 채널은 `판매 채널`, 이쪽은 `출고 방식`.
 *
...
export interface Item {
   ...
   */
  /**
   * @deprecated **걷어냈다**(2026-09-16). 켠 품목이 하나도 없다.
   *
   * "로트 합계 = 통틀어(벌크+낱개+박스), `stock` = 그중 벌크만" 이라는 우회였다.
   * 로트 관리의 제1원칙 — **재고 = 그 품목의 로트 잔량 합** — 을 일부러 끄는 것이라,
   * 켠 품목(볶음참깨)만 장부가 깨져 있었다(로트합 −520 vs stock 14 등).
   *
   * 품목마다 로트를 나눠 다는 이관이 끝나서 더는 필요 없다 — 포장분은 푸미푸드 로트,
   * 벌크는 청정식품 로트로 이미 갈려 있다. 읽는 코드는 없지만 **옛 문서에 남은 값을
   * 다시 살리지 않으려고** 칸만 남긴다.
   */
  lotsAreTotal?: never;
  ...
  /**
   * **까서 벌크로 되돌릴 수 있는 품목인가** — 16.5kg 캔처럼(2026-09-16 사장님:
   * "캔 종류를 보통 벌크로 까서 포장하는 경우가 많은데").
   *
   * 구성(BOM)만으로는 못 가린다. `참기름/병/350ml` 도 캔과 똑같이
   * `벌크 0.321L + 병 1 + 캡 1` 이라 운영 데이터로 재 보니 **158개**가 걸렸다.
   * 라벨 붙여 밀봉한 소매 병을 까서 벌크로 되돌리는 일은 없으므로, 켠 것만 깐다.
...
export interface Employee {
  ...
  fcmTokens?: string[];
  /**
   * **그 표가 어느 기기 것인가** — 표 글자 → `{ 기기 이름, 담은 날 }`.
   *
   * 2026-09-14: 알림이 안 온다는데 서버 로그는 `보냄 10/10` 이었다. 표가 글자뿐이라
   * **사무실 PC 에 남은 표인지 그분 폰 표인지 가릴 수가 없었다.** 안 보는 브라우저의 표도
   * FCM 은 영원히 성공으로 받아 주고, 알림은 그 PC 에 뜬다.
   *
   * `fcmTokens` 는 그대로 두고 옆에 딸린다 — 보내는 쪽(functions)이 그 모양에 기대고 있다.
   * 표가 죽어 지워져도 여기 남을 수 있으므로, 보여 줄 때는 **`fcmTokens` 에 있는 것만** 센다.
   */
  fcmDevices?: Record<string, { name: string; at: string }>;
}
...
export interface IssuedStatement {
  ...
  memo?: string;
  /**
   * **이 전표를 끊은 사람**(2026-09-15 사장님: "전표일자 다음에 담당자").
   *
   * 전표에는 원래 사람이 남지 않았다 — 누가 끊었는지 물어볼 데가 없어, 숫자가 이상하면
   * 물어볼 사람부터 찾아야 했다. 끊을 때와 고칠 때 찍는다.
   * **예전 전표는 비어 있다** — 없던 것을 지어내지 않는다.
   */
  createdBy?: string;
  /**
   * **세무 증빙을 무엇으로 했나** — 매출은 미발행·세금계산서·현금영수증,
   * 매입은 미수취·세금계산서·계산서(면세)·신용카드·현금영수증·간이영수증
   * (2026-09-15 사장님). 고를 것과 기본값은 `features/statements/domain/evidence` 가 안다.
   *
   * 안 골랐으면 `taxIssuedAt` 을 본다 — 이미 끊어 놓고 '미발행'으로 보이면 안 된다.
   */
  evidence?: string;

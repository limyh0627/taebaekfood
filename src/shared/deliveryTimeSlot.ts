 ...
 *
 * 값은 `settings/deliveryOrdering` 한 문서에 주문id → '오전'|'오후' 로 담긴다.
...
export const subscribeDeliveryOrdering = (callback: (doc: DeliveryOrderingDoc | null) => void) =>
  subscribeToDocument<DeliveryOrderingDoc>('settings', 'deliveryOrdering', callback);
 ...
 * 구분하려고 또 따져야 한다.
 */
export const saveDeliveryTimeSlot = (orderId: string, slot: DeliveryTimeSlot | null) =>
  setDocument('settings', 'deliveryOrdering', { timeSlots: { [orderId]: slot ?? deleteField() } });

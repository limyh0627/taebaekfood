...
describe('전표 만들기 전 검사', () => {
   ...
   */
  it('매입전표는 한쪽 줄만 있어도 통과한다 — 상대변은 분개가 세운다', () => {
    expect(statementBlockReason({ type: '매입', items: [
      li({ name: '전기세', accountCode: '520', side: '차변', total: 1_233_780 }),
    ] } as Partial<IssuedStatement>)).toBeNull();
  });

  it('매출전표도 마찬가지다', () => {
    expect(statementBlockReason(st([li({ accountCode: '800', side: '대변', total: 500_000 })]))).toBeNull();
  });
});
